...
function BasicInfo({ data }) {
    ...
    return (
        <div className="my-3">
            ...
            <ButtonAction text="Hiển thị tất cả" onClick={handleShow} className="mt-2" />
...
function Detail({ showAction = true, showReviews = true }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await axiosClient.get(`/rental_post/api/${roomId}/`);
                const rvs = await axiosClient.get(`/rental_post/api/by-posts/${roomId}/reviews/`);
    ...
    const rating = reviews?.reduce((acc, review) => acc + Number(review.rating), 0) / reviews?.length || 0;
    ...
    return (
        <div className={styles.container}>
            <div className="d-flex align-items-center justify-content-between mb-2">
                ...
                {showAction && (
                    <div className="d-flex align-items-center justify-content-end gap-1" style={{ width: '300px' }}>
                        <ButtonAction
